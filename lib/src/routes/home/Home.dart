...
class _HomeState extends State<Home> {
  ...
  Future money() async {
    response = await http.get("https://api.hgbrasil.com/finance?key=6b6695e0");
    var json = await jsonDecode(response.body);
    dolar = await json["results"]["currencies"]["USD"]["buy"];
    return dolar;
  }

  Future data(woeid) async {
    response = await http
  ...
  Future datas() async {
    await localizatio();
    await money();
    await data(localization["results"]["woeid"]);
    return {dolar, tempo, localization};
  }
